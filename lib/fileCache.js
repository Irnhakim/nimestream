...
export function deleteFileCache(key) {
  try {
    const filePath = getFilePath(key);
    if (fs.existsSync(filePath)) {
    ...
    console.error(`Error deleting file cache for key: ${key}`, error);
    return false;
  }
}
// cache helpers export: getFileCache, setFileCache, deleteFileCache
