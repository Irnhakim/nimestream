...
// In-memory cache for Kusonime results (TTL 5 minutes)
const kusoCache = new Map();
...
async function fetchKusonimeHtml(url) {
  ...
  try {
    ...
    // Check memory cache
    const now = Date.now();
    const cached = kusoCache.get(targetUrl);
...
export async function getLatestKusonime(page = 1) {
  ...
  try {
    ...
    $('.venz .kover').each((i, el) => {
      ...
      // Parse release date (e.g. from '<p><i class="fa fa-clock-o"></i> Released on 7:30 am</p>')
      let date = null;
      $(el).find('.content p').each((j, p) => {
        const text = $(p).text();
        if (text.includes('Released on')) {
          date = text.replace('Released on', '').trim();
        }
      });
    ...
    // Parse pagination (wp-pagenavi style)
    const pagination = [];
    $('.wp-pagenavi a, .wp-pagenavi span').each((i, el) => {
      const text = $(el).text().trim();
      const href = $(el).attr('href');
      let pageNum = null;

      if (href) {
        // Extract page number from URL (e.g. /page/2/ or /page/2)
      ...
      pagination.push({
        text,
        page: pageNum,
        active: $(el).hasClass('current') || $(el).attr('aria-current') === 'page',
        isExtend: $(el).hasClass('extend')
      });
    });
...
// Fetch full anime list index from Kusonime (Page 1 & 2 in parallel)
export async function getKusonimeAnimeList() {
  if (!KUSONIME_ENABLED) return {};
  try {
    const urls = [
      `${KUSONIME_URL}/list-anime-batch-sub-indo/`,
      `${KUSONIME_URL}/list-anime-batch-sub-indo/page/2/`
    ];

    const htmlContents = await Promise.all(urls.map(url => fetchKusonimeHtml(url)));
    const result = {};
    ...
    htmlContents.forEach(html => {
      ...
      $('.bariskelom').each((i, el) => {
        ...
        $(el).find('.penzbar .jdlbar ul li a').each((j, a) => {
          ...
          if (title && slug) {
            // Clean Kusonime title terms as requested
            const cleanTitle = title
              .replace(/\s*(?:BD\s*)?Batch\s*(?:Sub(?:title)?\s*(?:Indonesia|Indo)?)?/gi, '')
    ...
    console.error('Kusonime anime list fetch error:', error);
    return {};
  }
}
