...
// 1. Get Latest Episodes List from Oploverz (Ongoing updates)
export async function getLatestOploverz() {
  if (!OPLOVERZ_ENABLED) return [];
  const json = await fetchJson(`${BACK_API_URL}/episodes?limit=20`);
  if (!json || !json.data) return [];
  
  return json.data.map(item => {
    const series = item.series || {};
    // Use series poster or fallback to item cover
    const poster = series.poster || item.cover || '';
    const thumbUrl = poster.startsWith('http') ? poster : `https://backapi.oploverz.ac${poster}`;
    
    return {
      title: series.title || item.title || 'Ongoing',
      // Format slug as episode slug so home clicks go directly to player
      slug: `${series.slug || 'anime'}-episode-${item.episodeNumber}`,
      thumb: poster ? thumbUrl : '/placeholder.svg',
      ep: `Episode ${item.episodeNumber}`,
      date: item.releasedAt ? new Date(item.releasedAt).toLocaleDateString('id-ID') : 'Ongoing',
      source: 'Oploverz'
    };
  });
}
