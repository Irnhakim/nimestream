...
export const BASE_URL = process.env.NEXT_PUBLIC_OTAKUDESU_URL || 'https://otakudesu.blog';
export const LOCAL_API_URL = process.env.NEXT_PUBLIC_LOCAL_API_URL || 'http://localhost:3006';

// In-memory cache for scraping results (TTL 5 minutes)
...
export function sanitizeHtml(html) {
  ...
  $('script, iframe, style, object, embed, link, meta, input, button, form, img').remove();
  $('*').each((_, el) => {
    if (!el.attribs) return;
    const allowedAttribs = el.name === 'a' ? ['href'] : [];
    Object.keys(el.attribs).forEach(attr => {
      if (!allowedAttribs.includes(attr)) {
        $(el).removeAttr(attr);
      } else if (attr === 'href') {
...
export async function fetchHtml(url) {
  ...
  } else if (url.includes('otakudesu.blog')) {
    targetUrl = url.replace('https://otakudesu.blog', BASE_URL);
  }
...
export function parseAnimeDetails(html, slug) {
  ...
  let sinopsis = $('.sinopc').html()
    || $('.sinopse').html()
    || $('.cukder').next().next().html()
    || '';

  // Rewrite otakudesu links to nimestream links dynamically
  if (sinopsis) {
    // Sanitize scraped HTML to prevent XSS
    sinopsis = sanitizeHtml(sinopsis);

    // Replace full otakudesu domain paths to local ones
    sinopsis = sinopsis.replace(/href=["']https?:\/\/(?:www\.)?otakudesu\.[^"']+\/anime\/([^"']+)\/?["']/gi, 'href="/anime/$1"');
    sinopsis = sinopsis.replace(/href=["']https?:\/\/(?:www\.)?otakudesu\.[^"']+\/episode\/([^"']+)\/?["']/gi, 'href="/episode/$1"');

    // In case of post IDs (e.g., otakudesu.io/p/12345 or ?p=12345), redirect to search route
    sinopsis = sinopsis.replace(/href=["']https?:\/\/(?:www\.)?otakudesu\.[^"']+\/(?:\?p=|p\/)(\d+)\/?["']/gi, 'href="/search?q=$1"');

    // In case there are relative paths in html
    sinopsis = sinopsis.replace(/href=["']\/anime\/([^"']+)\/?["']/gi, 'href="/anime/$1"');
  }
  ...
  // Extract clean genres array for smart recommendations
  let genres = [];
  const genreRaw = info.genre || info.genres || '';
  if (genreRaw) {
    genres = genreRaw.split(',').map(g => g.trim());
  }
...
export function parseBatchDetails(html) {
  ...
  const downloads = [];
  $('.batchlink ul li, .download ul li, .download li').each((i, el) => {
...
export function parseEpisodeDetails(html) {
  ...
  // Parse thumbnail/cover image from meta og:image
  const thumb = $('meta[property="og:image"]').attr('content') || $('meta[name="twitter:image"]').attr('content') || null;
...
export function parseGenreAnimeList(html) {
  ...
  if (items.length === 0) {
    $('.chivsrc li').each((i, el) => {
      ...
      const slug = href ? href.split('/anime/')[1]?.replace(/\//g, '') : '';
      const rating = $(el).find('.set').eq(2).text().replace('Rating :', '').trim();
      const imgEl2 = $(el).find('img');
      const thumb = imgEl2.attr('src') || imgEl2.attr('data-src') || imgEl2.attr('data-lazy-src') || imgEl2.attr('data-original') || null;
