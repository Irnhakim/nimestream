...
export function parseHomeList(html, type = 'ongoing') {
  ...
  section.find('.detpost').each((i, el) => {
    ...
    items.push({ title, slug, ep, date, thumb });
  });
...
export function parseAnimeDetails(html, slug) {
  ...
  const thumb = $('.cukder img').attr('src');

  const sinopsis = $('.sinopse p').text().trim() || $('.cukder').next().next().text().trim();
  ...
  return {
    ...
    info,
    sinopsis,
    episodes: episodes.reverse() // Display oldest first or keep newest first (reverse list from page which has newest first)
  };
}
